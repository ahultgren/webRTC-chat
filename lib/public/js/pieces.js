(function (exports, $) {
  ...
  File.prototype.queue = function() {
    ...
    for(i = 0, l = this.queued.length; i < l; i++) {
      if(!this.queued[i].isRequested || this.queued[i].isSkipped > MAX_SKIPS) {
        app.rtc.send({}, { id: this.owner }, { id: this.peerId, piece: this.queued[i].index }, { op: 'request-piece' });
        this.queued[i].isRequested++;
        this.queued[i].isSkipped = 0;
      }
      else {
  ...
  exports.getFile = function (data, callback) {
    ...
    exports.files[name].complete.done(function () {
      // Remove the reference when it's completed
      delete exports.files[name];
    });
  };
